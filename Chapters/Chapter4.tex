...
% Data capture was completed using a Cartesian force applicator to generate the data required for the sensor performance quantification of spatial, temporal, and force sensor resolution.
\\
...
% insert table here comparing the different sensing technologies
...
% % Defining the material and piezoresistivity
% Correctly characterising PNEC materials through a known and replicable piezoresistive model enables their application in pressure sensing. The featured PNECs incorporate carbon black (CB) dispersed and suspended in a silicone rubber (SR) matrix, exhibiting varied CB percentages. Given silicone's widespread use as a biocompatible material in the biomedical industry, a method is introduced to modify the material's piezoresistivity, thereby adding a force-sensing capability. This modification expands the potential for numerous existing devices to possess a degree of mechanosensation.

% To ensure conduction through a perfectly homogeneous PNEC material the percolation threshold in terms of particle volume fraction must be reached or exceeded. To ensure a sensor domain is adequately sensitive throughout the PNEC volume, a volume fraction must be found that ensure a near optimal piezoresistivity. This volume fraction needs to be low enough to ensure the silicone matrix is well cross-linked avoiding viscous material effects and high enough such that the hardware driving the sensor drive signals through the material with a sufficiently low SNR \citep{Wu2019}. 

% To ensure piezoresistivity occurs with a high probability throughout the CBSR composite volume, the conduction percolation threshold of the system must be determined and a concentration of conductive particles higher than the percolation threshold must be mixed into the material. As well as reaching the conduction threshold which is found on a continuous domain, there is particle dispersion concentration at which the highest piezoresistivity is present, which is desirable for this application. Conduction within the PNECs is poised to occur through both traditional Coulomb conduction and quantum tunneling mechanisms \citep{Zare2017,Buketov2020,Spahr2017,Dong2017,Forestiere2012,Lux1993,Flandin1999}.
% \\

% Piezoresistivity within the PNEC material is caused by the adjacency of conductive particles within the material and how this adjacency changes in time \citep{Lux1993}. The relationship between the PNEC material's viscoelasticity and its piezoresistivity is not well defined, limiting the material's deployment as a pressure sensor. Researchers have shown that the magnitude of influence of the viscoelasticity on the piezoresistivity of these PNEC composite materials can be amplified by the following factors: temperature, material fatigue, humidity, radiation ageing, and high stress/strain conditions \citep{Klimm2022,AbuAbdeen2010}. However, how these viscoelastic effects directly affect the piezoresistivity in terms of particle movement is still unknown. 
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=8.5cm]{Figures/CBSR_8p_1_10push_XXstrain_60s_ts_vals_vs_strain_1D_sin_titulo.jpg}
%     \caption{1D mean settling times for 5 - 30\% compressive strain applied to CBSR 9 wt\% with the error bars showing the standard deviation.}
%     \label{fig:strain_ts_8p_1D}
% \end{figure}
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=8.5cm]{Figures/CBSR_9p_1_3push_XXstrain_240s_ts_vals_vs_strain_1D_sin_titulo.jpg}
%     \caption{Mean settling times for 5 - 30\% compressive strain applied to CBSR 9 wt\% with the error bars showing the standard deviation.}
%     \label{fig:strain_ts_9p_1D}
% \end{figure}
% TODO? - What about a pure silicone case?

% \subsection{Sensor Characterisation}\label{sec:Sensor Characterisation2}
% % TODO?: Sensor validation systems and EIT reconstruction parameters are quantified in this section.

% \subsubsection{Localised Stress Testing} \label{sec:Localised Stress Testing2}
% % TODO:

% \subsubsection{EIT Measurement}\label{sec:EIT Measurement2}
% % TODO:
...
% To determine whether the noise floor of the material in steady-state was sufficiently low the noise figure was calculated for the unloaded state of the material and given in Table \ref{table:DUT_nf_char}. 
...
\begin{figure}[H]
    ...
    \caption{Spatial performance metrics comparing threshold percentages of 25, 50, 75, and 85\% for a 8 wt\% CBSR sample being loaded with 20\% compressive strain in nine areas, $L_{0-8}$, shown in Figure \ref{fig:force_app_map}. The force time series plot data is light orange.} % TODO?: Determine better way of displaying these metrics??
...
%     \label{table:spatial_metrics_stats_8p}
%     \centering
...
%     \label{table:spatial_metrics_stats_9p}
%     \centering
%     \begin{tabular}{p{0.7cm}p{2cm}p{1.8cm}p{1.8cm}}
%     \hline
%         \% thresh & $E_{CoM}$ [mm] & $A_{OL}$ [\%] & $S\!D$ [mm$^2$]\\ \hline
...
The CBSR 8 wt\% samples gave better performance metric results than the 9 wt\% samples due to the residual transient effects of previous load events as exemplified in Figure \ref{fig:recon_perform_9p}. This will be mitigated in future by using a blob separation algorithm whereby each sensed-region/blob is given a weighting based on its appearance time, size, decay characteristic, and performance metric values.
